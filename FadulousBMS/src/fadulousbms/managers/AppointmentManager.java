...
public class AppointmentManager extends BusinessObjectManager
{
    ...
    public void newWindow()
    {
        ...
        if(smgr.getActive()!=null)
        {
            if(!smgr.getActive().isExpired())
            {
                ...
                stage.setTitle(Globals.APP_NAME.getValue() + " - Appointment Documents");
                ...
                tblAppointment.setItems(lst_inspection);
                tblAppointment.getColumns().addAll(index, label, document, action, required, mark);
                ...
                //Menu - File
                MenuItem new_item = new MenuItem("New Appointment document reference");
                new_item.setOnAction(event -> handleNewAppointmentReference(stage));
                ...
                generate_index.setOnAction(event -> IO.viewIndexPage("Appointment Documents Index", documents, "bin/appointment_index.pdf"));

                //Menu - Print
                MenuItem print_index = new MenuItem("Print Index Page");
                print_index.setOnAction(event -> IO.printIndexPage("bin/appointment_index.pdf"));
                ...
                border_pane.setCenter(tblAppointment);
    ...
    public void handleNewAppointmentReference(Stage parentStage)
    {
        parentStage.setAlwaysOnTop(false);
        Stage stage = new Stage();
        stage.setTitle(Globals.APP_NAME.getValue() + " - New Appointment Document Reference");
        ...
        submit = CustomTableViewControls.getSpacedButton("Submit", event ->
        {
            ...
            try
            {
                ...
                if(connection!=null)
                {
                    ...
                    {
                        JOptionPane.showMessageDialog(null, "Successfully added new appointment document reference!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }else{
