...
public class OHSManager extends BusinessObjectManager
{
    ...
    public void newWindow()
    {
        ...
        if(smgr.getActive()!=null)
        {
            if(!smgr.getActive().isExpired())
            {
                ...
                stage.setTitle(Globals.APP_NAME.getValue() + " - OHS Act Documents");
                ...
                tblOHS.getColumns().addAll(index, label, document, action, required, mark);
                ...
                //Menu - File
                MenuItem new_item = new MenuItem("New OHS document reference");
                new_item.setOnAction(event -> handleNewOHSReference(stage));
                ...
                generate_index.setOnAction(event -> IO.viewIndexPage("OHS Documents Index", documents, "bin/ohs_index.pdf"));

                //Menu - Print
                MenuItem print_index = new MenuItem("Print Index Page");
                print_index.setOnAction(event -> IO.printIndexPage("bin/ohs_index.pdf"));
    ...
    public void handleNewOHSReference(Stage parentStage)
    {
        parentStage.setAlwaysOnTop(false);
        Stage stage = new Stage();
        stage.setTitle(Globals.APP_NAME.getValue() + " - New OHS Document Reference");
        ...
        submit = CustomTableViewControls.getSpacedButton("Submit", event ->
        {
            ...
            try
            {
                ...
                if(connection!=null)
                {
                    ...
                    {
                        JOptionPane.showMessageDialog(null, "Successfully added new OHS document reference!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    }else{
