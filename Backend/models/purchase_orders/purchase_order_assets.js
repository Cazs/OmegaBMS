...
const purchaseOrderItemSchema = mongoose.Schema(
  ...
  const PurchaseOrderItems = module.exports = mongoose.model('purchase_order_assets', purchaseOrderItemSchema);
  ...
  {
    console.log('attempting to create new Purchase Order Asset for Purchase Order [%s].', purchase_order_item.purchase_order_id);
    PurchaseOrderItems.create(purchase_order_item, function(error, res_obj)
    {
      ...
      console.log('successfully created new purchase_order_asset.')
      if(callback)
  ...
  module.exports.update = function(record_id, purchase_order_item, callback)
  {
    ...
    console.log('attempting to update purchase_order_asset [%s].', record_id);
    PurchaseOrderItems.findOneAndUpdate(query, purchase_order_item, {}, function(error, res_obj)
    {
      ...
      console.log('successfully updated purchase_order_asset.')
      if(callback)
        callback(error, res_obj);
      //update timestamp
      counters.timestamp('purchase_orders_timestamp');
  ...
  {
    console.log('validating purchase_order_asset object:\n%s', JSON.stringify(purchase_order_item));
    ...
    console.log('valid purchase_order_asset object.');
